(function($) {
		...
		success:function(res){
			if(res.status=="01"){
				...
				for(var i=0;i<res.rows.length;i++){
					if(res.rows[i].sysId == locationUrl.split("&")[0].split("=")[1]){
						flag = i;
					}
				}
				$.ajax({
					...
					success:function(ress){
						if(ress.status=="01"){
							for(var i=0;i<res.rows.length;i++){
								if(i==0){
									$("#my-video").attr("src",ress.rows[0].videoAddr)
									$(".player-list-box").append(
										'<div class="player-list-video video-now">'+ress.rows[0].name+'</div>'
									)
									videoListAll.push(ress.rows[0].videoAddr)
								}else{
									if(flag == i){
										$(".player-list-box").append(
											'<div class="player-list-video">'+res.rows[0].name+'</div>'
										)
										videoListAll.push(res.rows[0].videoAddr)
									}else{
										$(".player-list-box").append(
											'<div class="player-list-video">'+res.rows[i].name+'</div>'
										)
										videoListAll.push(res.rows[i].videoAddr)
									}
								}
							}
						}
    ...
    video.on('timeupdate', function() {
        ...
        if($('.psVideo-timeBar').css('width')==$('.psVideo-progress').css('width')){
        	$('.psVideo-play-btn').removeClass('psVideo-play').addClass('psVideo-stop');
        }
    });
